# 📊 Kibana Installation Guide (Tarball-Based)
...
cd ~/
wget https://artifacts.elastic.co/downloads/kibana/kibana-$VERSION_NUMBER-linux-x86_64.tar.gz
wget https://artifacts.elastic.co/downloads/kibana/kibana-$VERSION_NUMBER-linux-x86_64.tar.gz.sha512
shasum -a 512 -c kibana-$VERSION_NUMBER-linux-x86_64.tar.gz.sha512
tar -xzf kibana-$VERSION_NUMBER-linux-x86_64.tar.gz
```
...
```bash
mkdir /es && cd /es
mv ~/kibana-$VERSION_NUMBER_NUMBER /es/
mkdir /es/kibana-$VERSION_NUMBER/logs
ln -s /es/kibana-$VERSION_NUMBER /es/kibana
chown -R kibana:kibana /es/kibana
```
...
```bash
mv /es/kibana/config/kibana.yml /es/kibana/config/kibana.yml.org
nano /es/kibana/config/kibana.yml
```
...
## 6️⃣ Systemd Service for Kibana
...
* ✅ Kibana is installed
* ✅ Kibana is connected to Elasticsearch
* ✅ UI is accessible
