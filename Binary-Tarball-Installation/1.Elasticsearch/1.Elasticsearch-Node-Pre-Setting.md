# 🛠️ Elasticsearch Node Pre-Requisites (Tarball Deployment)
...
➡️ Replace hostname accordingly:
- `es2.apsis.localnet`
- `es3.apsis.localnet`

---

## 🌐 2. Host Resolution (`/etc/hosts`)
...
## ⚙️ 5. Kernel Parameter (`vm.max_map_count`)
...
## 🚫 7. Ensure `/var/tmp` is Executable (CRITICAL)
...
➡️ Remove `noexec` from `/etc/fstab` permanently.
...
## 👤 11. Create Elasticsearch System User
...
👉 Proceed with:
- Elasticsearch tarball installation
- TLS configuration
- systemd unit setup
- Cluster bootstrap
